import {CAREER_JOBS} from './careers.js';
...
export function taskStatus(s,w,elapsed=0){
 ...
 if(CAREER_JOBS[job]){const progress=(w.careerProgress||0)+extra*1000;return {...out,detail:`${CAREER_JOBS[job].skill} · produces or processes materials every 4 minutes`,fraction:Math.min(1,progress/240000),remaining:Math.max(0,(240000-progress)/1000),cycle:240};}
 if(job==='forest')return {...out,detail:`${s.forestBattles?.[w.id]?.enemies[0]?.name||'Scouting'} · ${(w.stats.forestKills||0)} victories`,fraction:(w.forestProgress||0)/90000,remaining:(90000-(w.forestProgress||0))/1000,cycle:90};
 if(job==='rescued')return {...out,detail:`Recovery costs ${w.forestRescue||0} gold`,blocked:true};
